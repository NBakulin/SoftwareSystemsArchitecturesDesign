...
class Program
{
    static void Main(string[] args)
    {
        ...
        customer.SendAll("Всем");

        List<Colleague> c = new List<Colleague>();
        c.Add(customer);
        c.Add(programmer);
        customer.SendTo("Прогеру и заказчику", c);
...
abstract class Mediator
{
    ...
    public abstract void SendAll(string msg);//new
    public abstract void SendTo(string msg, List<Colleague> collegues);
}
abstract class Colleague
{
    ...
    public virtual void SendAll(string message)//New
    {
        mediator.SendAll(message);
    }
    public virtual void SendTo(string message, List<Colleague> colleagues)
    {
        mediator.SendTo(message, colleagues);
    }
...
class ManagerMediator : Mediator
{
    ...
    public override void SendAll(string msg)//New
    {
        Programmer.Notify(msg);
        Tester.Notify(msg);
        Customer.Notify(msg);
    ...
    public override void SendTo(string msg, List<Colleague> collegues)
    {
        if (collegues != null && collegues.Count != 0)
        {
            foreach (Colleague c in collegues)
            {
                c.Notify(msg);
            }
        }
